...
/*
  ...
  License: GPLv3
 */
include('ncc_classify.php');
include('ncc_notification.php');
$ncc_send_spam_comment_to_admin = true;


$ncc_spam_settings = array(
    'ncc_send_spam_comment_to_admin' => $ncc_send_spam_comment_to_admin,
    'ncc_admin_email' => get_option('admin_email')
);

function ncc_spam_enqueue_script() {
    if (is_singular() && comments_open()) { 
        wp_enqueue_script('ncc_script', plugins_url('/ncc_spamfilter.js', __FILE__), array('jquery'), null, true);
    }
}
...
function ncc_spam_check_comment($commentdata) {
    ...
    if ($ncc_spam_flag) { // it is spam
        ...
        if ($ncc_spam_settings['ncc_send_spam_comment_to_admin']) {
            ...
            @wp_mail($ncc_spam_settings['admin_email'], $ncc_subject, $ncc_message); // send spam comment to admin email
        }
        ncc_spam_log_stats();
        wp_die($ncc_pre_error_message . $ncc_error_message); // die - do not send comment and show errors
    } else {
        $ncc_expr1 = preg_replace('/[^A-Za-z0-9\-\']/', ' ', $comment_content);
        $ncc_expr2 = strtolower($ncc_expr1);
        $ncc_spam_prob = ncc_classifier($ncc_expr2);
