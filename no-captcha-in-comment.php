<?php
/*
  Plugin Name: No Captcha in Comments
  Plugin URI: http://buffercode.com/no-captcha-in-comments/
...
function ncc_spam_check_comment($commentdata) {
    ...
    if ($ncc_spam_flag) { // it is spam
        $ncc_error_message .= '<strong>Comment was blocked because it is bot spam.</strong><br> ';
        if ($ncc_spam_settings['ncc_send_spam_comment_to_admin']) {
            $ncc_subject = 'Spam comment on site [' . get_bloginfo('name') . ']'; // email subject
            $ncc_message = 'Comment Blocked:'.$comment_content;
            if(isset($_POST['name'])){
                $ncc_message .= $_POST['name'];
                }
                if(isset($_POST['email'])){
                $ncc_message .= $_POST['email'];
                }
                if(isset($_POST['url'])){
                $ncc_message .= $_POST['url'];
                }
        ...
        if ($ncc_flag) { // it is spam
            $ncc_error_message .= '<strong>Comment was blocked because it is manual spam.</strong><br> ';
            if ($ncc_spam_settings['ncc_send_spam_comment_to_admin']) {
                $ncc_subject = 'Spam comment on site [' . get_bloginfo('name') . ']'; // email subject
                $ncc_message = 'Comment Blocked:'.$comment_content;
            if(isset($_POST['name'])){
